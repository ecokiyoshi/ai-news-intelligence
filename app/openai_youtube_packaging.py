"""OpenAI-backed YouTube title/thumbnail generation and evaluation."""
...
class OpenAIYouTubePackagingDraft(BaseModel):
...
class OpenAIYouTubePackagingGenerationResponse(BaseModel):
    candidates: list[OpenAIYouTubePackagingDraft] = Field(min_length=1)


OpenAIYouTubePackagingResponse = OpenAIYouTubePackagingGenerationResponse


class OpenAIYouTubePackagingDimension(BaseModel):
    candidate_index: StrictInt = Field(ge=0)
...
class OpenAIYouTubePackagingEvaluationResponse(BaseModel):
    evaluations: list[OpenAIYouTubePackagingDimension] = Field(min_length=1)
...
class OpenAIYouTubePackagingGenerator:
    """Generate title/thumbnail drafts with typed Responses API output."""
    ...
    def generate(
        ...
        *,
        ...
        response = self.client.responses.parse(
            model=self.model,
            instructions=YOUTUBE_PACKAGING_GENERATION_INSTRUCTIONS,
            input=json.dumps(
                {
                    "channel_focus": focus,
            ...
            text_format=OpenAIYouTubePackagingGenerationResponse,
        )
        parsed = _parsed_output(
            response, "OpenAI response did not contain parsed packaging candidates"
        )
        drafts = [
            YouTubePackagingDraft(
...
class OpenAIYouTubePackagingEvaluator:
    """Evaluate packaging dimensions with typed Responses API output."""
    ...
    def evaluate(
        ...
        *,
        ...
        response = self.client.responses.parse(
            model=self.model,
            instructions=YOUTUBE_PACKAGING_EVALUATION_INSTRUCTIONS,
            input=json.dumps(
                {
                    "channel_focus": focus,
            ...
            text_format=OpenAIYouTubePackagingEvaluationResponse,
        )
        parsed = _parsed_output(
            response, "OpenAI response did not contain parsed packaging evaluations"
        )
        dimensions = [
            YouTubePackagingDimensions(
